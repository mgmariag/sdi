...
def run_anfis_daily_irrigation(
    start_date: date,
    end_date: date,
    model: ANFIS | modeling.AnfisModelController,
    persist: bool,
    ...
    while current_date <= end_date:
        ...
        zone_signal_count = 0
        zone_signal_max = 0.0
        current_decision_threshold = modeling.anfis_decision_threshold(
            sensor_context,
            current_date,
            decision_threshold,
        ...
        for hour_weather in day_weather:
            ...
            for pot in pots:
                ...
                if slot is None:
                    if int(pot["id"]) in control_pot_ids and is_emergency_dryness(state, pot, current_date, observed_local):
                        if record_date:
            ...
            if slot is not None:
                for pot in control_pots:
                    ...
                    anfis_input = modeling.anfis_inputs(
                        state,
                        hour_weather,
                        None,
                        pot,
                    ...
                    rule_decision = modeling.make_anfis_execution_decision(state, pot, hour_weather, day_profile, slot)
                    predicted_probability = modeling.predict_anfis_probability(model, anfis_input, pot.get("balcony_zone"))
                    ...
                    safety_threshold = threshold_for_pot(pot, day_profile, slot)
            ...
            if slot is not None:
                ...
                zone_probability_summary = {
                    zone: modeling.anfis_zone_probability_summary(zone_decisions)
                    ...
                    if zone_activation_probability >= current_decision_threshold:
                        for decision in eligible_decisions:
                            ...
                            decision["reason_detail"] = (
                                ...
                                f"{modeling.anfis_duration_policy_note(decision)}"
                            )
