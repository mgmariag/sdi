...
from digital_twin.simulation.anfis.execution import AnfisDailyIrrigationRunner
...
from digital_twin.simulation.daily_irrigation import DefaultDailyIrrigationRunner
...
class SimulationEngine:
    """Coordinates experiment simulation workflows."""

    def __init__(self, snapshot_loader: ExperimentSnapshotLoader | None = None) -> None:
        self.snapshot_loader = snapshot_loader or ExperimentSnapshotLoader()
    ...
    def _resolve_snapshot(
        ...
        if snapshot is None:
            return self.snapshot_loader.load(start_date, end_date)
    ...
    def _resolve_anfis_training_snapshot(
        ...
        training_start = min(start_date, self.state_simulation_start(end_date, end_date))
        try:
            return self.snapshot_loader.load(training_start, end_date)
        except ValueError:
            return fallback_snapshot

    def state_simulation_start(self, start_date: date, end_date: date) -> date:
        return state_window.state_simulation_start(start_date, end_date, self.historical_state_anchor_date)
    ...
    def resolve_simulation_snapshot(
        ...
        simulation_start_date = self.state_simulation_start(start_date, end_date)
        if simulation_start_date >= start_date:
            return start_date, selected_snapshot

        try:
            return simulation_start_date, self.snapshot_loader.load(simulation_start_date, end_date)
        except ValueError:
            return start_date, selected_snapshot
    ...
    def _strategy_comparison_baseline(
        ...
        return self.run_default_dt_irrigation_control(
    ...
    def run_default_dt_irrigation_control(
        ...
        selected_snapshot = self._resolve_snapshot(start_date, end_date, snapshot)
        if state_anchor_policy == "experiment_start_sensor_anchor":
            simulation_start_date = start_date
            simulation_snapshot = selected_snapshot
        else:
            simulation_start_date, simulation_snapshot = self.resolve_simulation_snapshot(
                start_date,
                end_date,
                selected_snapshot,
            )

        return DefaultDailyIrrigationRunner(
            start_date=start_date,
            end_date=end_date,
            persist=persist,
    ...
    def run_daily_sampling_experiment(
        ...
        snapshot = self._resolve_snapshot(start_date, end_date, snapshot)
        ...
        baseline = baseline_result or self.run_default_dt_irrigation_control(
            start_date=start_date,
            end_date=end_date,
            persist=False,
            snapshot=snapshot,
        )
        sparse = self.run_sparse_daily_irrigation_with_snapshot(
            start_date=start_date,
            end_date=end_date,
        ...
        summary = {
            ...
            **comparison.decision_counts(),
            ...
            "sparse_moisture_display_policy": "baseline_synced_at_sample_points_simulated_between_samples",
    ...
    def run_daily_fuzzy_dt_experiment(
        ...
        start_time = perf_time.perf_counter()
        snapshot = self._resolve_snapshot(start_date, end_date, snapshot)
        baseline = self._strategy_comparison_baseline(start_date, end_date, snapshot, baseline_result)
        fuzzy = self.run_fuzzy_dt_daily_irrigation_with_snapshot(
            start_date=start_date,
            end_date=end_date,
            persist=persist,
            snapshot=snapshot,
    ...
    def run_daily_anfis_experiment(
        ...
        return anfis_experiment.run_daily_anfis_experiment(
            ...
            end_date=end_date,
            selected_snapshot=selected_snapshot,
            training_snapshot_resolver=self._resolve_anfis_training_snapshot,
            strategy_comparison_baseline=self._strategy_comparison_baseline,
            anfis_daily_runner=self.run_anfis_daily_irrigation_with_snapshot,
            seed=seed,
    ...
    def run_fuzzy_dt_daily_irrigation_with_snapshot(
        ...
        selected_snapshot = self._resolve_snapshot(start_date, end_date, snapshot)
        return FuzzyDailyIrrigationRunner(
            start_date=start_date,
            end_date=end_date,
            persist=persist,
            selected_snapshot=selected_snapshot,
    ...
    def run_anfis_daily_irrigation_with_snapshot(
        ...
        selected_snapshot = self._resolve_snapshot(start_date, end_date, snapshot)
        return AnfisDailyIrrigationRunner(
            start_date=start_date,
            end_date=end_date,
            model=model,
            persist=persist,
