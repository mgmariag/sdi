...
from digital_twin.simulation.irrigation_controller.fuzzy_decision import make_fuzzy_dt_decision
    ...
    def allows_heatwave_supplement(
        ...
        return (
            ...
            and (
                ...
                or number(day_profile.get("max_temperature_c"), 20.0) >= 35.0
            )
        )
...
def run_fuzzy_dt_daily_irrigation(
    start_date: date,
    end_date: date,
    persist: bool,
    ...
    pot_states = copy_pot_states(simulation_snapshot.initial_pot_states)
    sensor_state_anchor = initialize_states_from_first_day_sensor_readings(
        pot_states,
        pots,
        sensor_context,
    ...
    while current_date <= end_date:
        ...
        day_profile = simulation_snapshot.day_profiles.get(current_date) or build_day_profile(current_date, day_weather, weather_by_day)
        daily_water_ml = 0.0
        daily_events = 0
        daily_decisions = 0
        daily_alerts = 0
        ...
        for hour_weather in day_weather:
            ...
            if slot is not None:
                for pot in control_pots:
                    state = pot_states[pot["id"]]
                    decision = make_fuzzy_dt_decision(state, pot, hour_weather, day_profile, slot)
