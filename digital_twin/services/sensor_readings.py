...
class ActuationFeedbackService:
    ...
    def _delivery_items(self, actuation: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        actual_delivered_ml = _number(
            actuation.get("delivered_volume_ml"),
            _number(actuation.get("planned_volume_ml"), 0.0),
        )
            ...
            base_items = [
                {
                    ...
                    "delivered_volume_ml": max(
                        0.0,
                        _number(item.get("delivered_volume_ml"), item.get("planned_volume_ml", 0.0)),
                    ),
                }
    ...
    def _advance_state(self, state: dict[str, Any], pot: dict[str, Any], target_at: datetime) -> None:
        ...
        _apply_hourly_environment(state, pot, weather, target_at.date(), hours=hours)
    ...
    def _corrective_prescription(
        ...
        valve_number = _valve_number_for_zone(str(pot["balcony_zone"]))
        payload = {
...
def _is_hot_irrigation_day(conn, day: date) -> bool:
    ...
    return bool(row and _number(row["max_temperature_c"], 0.0) >= 32.0)
