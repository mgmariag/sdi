...
LOCATION = dict(DEFAULT_WEATHER_LOCATION)
...
def hourly_request_params(start: date, end: date) -> dict[str, Any]:
    return {
...
def forecast_request_params(start: date, end: date, today: date | None = None) -> dict[str, Any]:
    local_today = today or today_local()
    past_days = max(0, min((local_today - start).days, FORECAST_PAST_DAYS_MAX))
    forecast_days = max(1, min((end - local_today).days + 1, FORECAST_DAYS))
    return {
        "latitude": LOCATION["latitude"],
        "longitude": LOCATION["longitude"],
        "past_days": past_days,
...
def weather_rows_from_payload(payload: dict[str, Any], source: str, is_forecast: bool) -> list[dict[str, Any]]:
    hourly = payload.get("hourly") or {}
    times = hourly.get("time") or []
    rows = []
    for index, timestamp in enumerate(times):
        raw_values = {name: _series_value(hourly, name, index) for name in HOURLY_VARIABLES}
        raw_payload = {name: json_ready(value) for name, value in raw_values.items()}
        observed_local_at = local_bucket_from_open_meteo(timestamp)
        rows.append(
            {
                "location_name": LOCATION["location_name"],
                "latitude": Decimal(str(LOCATION["latitude"])),
                "longitude": Decimal(str(LOCATION["longitude"])),
                "observed_at": local_datetime_from_open_meteo(timestamp),
...
def local_datetime(day: date) -> datetime:
    return datetime(day.year, day.month, day.day, tzinfo=ZoneInfo(LOCATION["timezone"]))
...
def local_bucket_from_observed_at(value: datetime) -> datetime:
    if value.tzinfo is None:
        return value
    return value.astimezone(ZoneInfo(LOCATION["timezone"])).replace(tzinfo=None)


def local_bucket_from_open_meteo(value: str) -> datetime:
    parsed = datetime.fromisoformat(value)
    if parsed.tzinfo:
        return parsed.astimezone(ZoneInfo(LOCATION["timezone"])).replace(tzinfo=None)
    return parsed


def local_datetime_from_open_meteo(value: str) -> datetime:
    parsed = datetime.fromisoformat(value)
    if parsed.tzinfo:
        return parsed
    return parsed.replace(tzinfo=ZoneInfo(LOCATION["timezone"]))
...
def _csv_hourly_row(header: list[str], raw_row: list[str], metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_payload = {
        _normalize_csv_column(column): json_ready(_csv_decimal(value))
        for column, value in values.items()
        if column != "time"
    }
    ...
    return {
        "location_name": LOCATION["location_name"],
        "latitude": Decimal(str(metadata.get("latitude") or LOCATION["latitude"])),
        "longitude": Decimal(str(metadata.get("longitude") or LOCATION["longitude"])),
