...
class AnfisModelService:
    ...
    def __init__(
        self,
        repository: AnfisModelRepository | None = None,
    ...
    def train_if_needed(
        ...
        *,
        ...
        if not force and latest and not self.needs_training(latest, sensor_watermark, config):
            return {
                "status": "skipped",
        ...
        start_date, end_date = self.resolve_training_range(start_date, end_date)
        logger.info(
    ...
    def needs_training(
        ...
        if int(metrics.get("training_dataset_version") or 0) != int(config.get("training_dataset_version") or 0):
            return True
        if self._stored_input_features(latest) != list(config.get("anfis_input_features") or []):
            return True
    ...
    def resolve_training_range(self, start_date: date | None, end_date: date | None) -> tuple[date, date]:
        if end_date is None:
            with get_connection() as conn:
                end_date = conn.execute("SELECT max(observed_date) FROM weather_hourly").fetchone()[0]
        end_date = end_date or self.clock.today()
        return start_date or self.clock.add_months(end_date, -DEFAULT_ANFIS_TRAINING_MONTHS), end_date
