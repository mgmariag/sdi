    ...
    with get_connection(row_factory=dict_row) as conn:
        weather = conn.execute(
            ...
            SELECT
                ...
                count(*) AS row_count,
                min(observed_at) AS first_timestamp,
                max(observed_at) AS last_timestamp
            FROM weather_hourly
...
def get_weather_hourly(start: date, end: date, limit: int = 1000) -> list[dict[str, Any]]:
    initialize_database()
    start_dt = _local_datetime(start)
    end_dt = _local_datetime(end + timedelta(days=1))
    with get_connection(row_factory=dict_row) as conn:
        rows = conn.execute(
            """
            WITH ranked_weather AS (
                ...
                FROM weather_hourly
                WHERE location_name = %(location)s
                  AND observed_at >= %(start_dt)s
                  AND observed_at < %(end_dt)s
            )
            SELECT
                id,
                location_name,
...
def _weather_upsert_query() -> str:
    ...
    updates = ", ".join(
        ...
        for column in WEATHER_INSERT_COLUMNS
        if column not in {"location_name", "source", "observed_at"}
    )
    ...
    return f"""
        ...
        VALUES ({values})
        ON CONFLICT (location_name, source, observed_at) DO UPDATE SET
            {updates},
            changed_at = now()
