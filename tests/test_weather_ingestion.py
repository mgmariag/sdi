...
class WeatherIngestionTests(TestCase):
    def test_refresh_start_uses_latest_change_minus_lookback(self) -> None:
        ...
        reader = WeatherCacheReader()

        with patch.object(reader.open_meteo, "today_local", return_value=date(2026, 6, 6)):
            refresh_start = reader.refresh_start_from_latest_change(latest_change)
    ...
    def test_refresh_start_falls_back_to_today_minus_lookback(self) -> None:
        ...
        with patch.object(reader.open_meteo, "today_local", return_value=date(2026, 6, 6)):
            refresh_start = reader.refresh_start_from_latest_change(None)
    ...
    def test_forecast_request_uses_past_days_and_full_forecast_days(self) -> None:
        ...
        with patch.object(ingestion.open_meteo, "today_local", return_value=date(2026, 6, 6)):
            params = ingestion._forecast_request_params(date(2026, 5, 28), date(2026, 6, 21))
