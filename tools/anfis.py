        ...
        for temp_idx in range(3):
            for hum_idx in range(3):
                for moisture_idx in range(3):
                    yield moisture_idx, hum_idx, temp_idx

    def predict(self, inputs: Dict[str, float]) -> float:
        memberships = self._unpack_membership(self.membership_params)
        weights = []
        outputs = []
        for rule_index, (m_idx, h_idx, t_idx) in enumerate(self._rule_indices()):
            m_val = gaussian(inputs["moisture"], *memberships["moisture"][m_idx])
            h_val = gaussian(inputs["humidity"], *memberships["humidity"][h_idx])
            t_val = gaussian(inputs["temperature"], *memberships["temperature"][t_idx])
            weight = m_val * h_val * t_val
            weights.append(weight)
            outputs.append(weight * self.rule_outputs[rule_index])
        total_weight = sum(weights)
        if total_weight <= 0.0:
            return float(sum(self.rule_outputs)) / max(len(self.rule_outputs), 1)
        return float(sum(outputs) / total_weight)
