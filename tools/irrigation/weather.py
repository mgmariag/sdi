...
from tools.irrigation.models import LOCAL_TZ, LOCATION_NAME
from tools.irrigation.utils import local_observed_at as _local_observed_at, number as _number
...
def _raise_if_missing_historical_weather(weather_rows: list[dict[str, Any]], start_date: date, end_date: date) -> None:
    ...
    message = f"No stored historical weather data is available for {requested_range['start']} to {requested_range['end']}."
    raise NoWeatherData(
        message,
        {
...
def _missing_historical_weather_hours(weather_rows: list[dict[str, Any]], start_date: date, end_date: date) -> list[datetime]:
    ...
    existing_hours = {
        _local_observed_at(row).replace(minute=0, second=0, microsecond=0)
    ...
    with get_connection(row_factory=dict_row) as conn:
        rows = conn.execute(
            ...
            WITH weather_dates AS (
                SELECT DISTINCT
                    (observed_at AT TIME ZONE 'Europe/Bucharest')::date AS weather_date
                FROM weather_hourly
                WHERE location_name = %(location)s
            ),
            numbered_dates AS (
